...
If you set `ANTHROPIC_BASE_URL`, the container runner injects `ANTHROPIC_BASE_URL`, `ANTHROPIC_AUTH_TOKEN`, and `ANTHROPIC_API_KEY` (when present) directly into the Claude container and skips OneCLI's credential proxy so requests go straight to your endpoint. Leave `ANTHROPIC_BASE_URL` unset to continue using OneCLI-managed credentials.

For OpenAI-compatible targets (Ollama, llama.cpp, etc.) run [anthropic-proxy](https://github.com/maxnowack/anthropic-proxy) alongside NanoClaw. Start the proxy with `ANTHROPIC_PROXY_BASE_URL=http://localhost:8001/v1 npm run proxy` (pointing to your OpenAI-style backend) and set `ANTHROPIC_BASE_URL=http://localhost:3000` so NanoClaw routes through the proxy. The proxy converts Anthropic tool calls into OpenAI-compatible requests before forwarding them.
