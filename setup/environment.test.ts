...
const credentialPattern =
  /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|ANTHROPIC_AUTH_TOKEN|ANTHROPIC_BASE_URL|ONECLI_URL)=/m;
...
describe('credentials detection', () => {
  it('detects ANTHROPIC_API_KEY in env content', () => {
    ...
    const hasCredentials = credentialPattern.test(content);
  ...
  it('detects ANTHROPIC_AUTH_TOKEN in env content', () => {
    const content = 'ANTHROPIC_AUTH_TOKEN=my-lab-token';
    const hasCredentials = credentialPattern.test(content);
    expect(hasCredentials).toBe(true);
  });

  it('detects ANTHROPIC_BASE_URL override in env content', () => {
    const content = 'ANTHROPIC_BASE_URL=http://localhost:9000';
    const hasCredentials = credentialPattern.test(content);
    expect(hasCredentials).toBe(true);
  });
